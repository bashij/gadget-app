...
RSpec.describe 'TweetLikes', type: :request do
  ...
  describe 'POST #create' do
    context 'ログインしていない状態' do
      specify 'ログイン画面へ遷移するための情報を返す' do
        post "/api/v1/tweets/#{tweet.id}/tweet_likes", params: { tweet_id: tweet.id }
  ...
  describe 'DELETE #destroy' do
    context 'ログインしていない状態' do
      specify 'ログイン画面へ遷移するための情報を返す' do
        delete "/api/v1/tweets/#{tweet.id}/tweet_likes", params: { tweet_id: tweet.id }
        json = JSON.parse(response.body)
    ...
    context 'ログインしている状態' do
      ...
      context '失敗の場合' do
        specify 'ログインユーザー以外のツイートへのいいねを削除しようとすると操作失敗の情報を返す' do
          delete "/api/v1/tweets/#{other_tweet.id}/tweet_likes", params: { tweet_id: other_tweet.id }
          json = JSON.parse(response.body)
