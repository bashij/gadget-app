class Gadget < ApplicationRecord
  ...
  validates :category, presence: true, length: { maximum: 20 },
                       inclusion: { in: %w[ PC本体 オーディオ キーボード スマートウォッチ スマートフォン スマート家電 タブレット
                                            チェア デスク ノートパソコン マウス モニター 充電器 その他 ] }
  validates :model_number, length: { maximum: 40 }
  validates :manufacturer, length: { maximum: 40 }
  validates :price, numericality: { allow_nil: true, greater_than_or_equal_to: 0, less_than_or_equal_to: 9_999_999 }
  validates :other_info, length: { maximum: 40 }
