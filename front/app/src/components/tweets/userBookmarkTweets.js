...
export default function UserBookmarkTweets(props) {
  const API_ENDPOINT = process.env.NEXT_PUBLIC_API_ENDPOINT_USERS
  const [pageIndex, setPageIndex] = useState(1)
  const { mutate } = useSWRConfig()
  const { data, error, isLoading } = useSWR(
    `${API_ENDPOINT}/${props.pageUser.id}/user_bookmark_tweets?paged=${pageIndex}`,
    fetcher,
    {
  ...
  useEffect(() => {
    if (recordCount) {
      props.setUserBookmarkTweetCount(recordCount)
    }
  }, [])
                ...
                {data?.tweets?.map((tweet) => {
                  return (
                    <Tweet
                      ...
                      mutate={mutate}
                      swrKey={`${API_ENDPOINT}/${props.pageUser.id}/user_bookmark_tweets?paged=${pageIndex}`}
